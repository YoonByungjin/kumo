...
| 機能 | 実装詳細 |
|------|----------|
| ホーム・ログイン | サービスランディングページおよびAjaxベースのログイン画面実装 |
| 会員登録（求職者） | 基本情報入力 + メール重複確認（Ajax） → 登録即時有効化 |
| 会員登録（求人者） | 基本情報 + 事業証明書類添付 → 登録後`isActive=false`状態。Adminが書類確認後に手動承認するワークフロー |
| ID検索 | 求職者/求人者タブ分離、氏名＋携帯番号でメール検索 |
| PW検索・変更 | 氏名＋携帯番号＋メール入力 → メール認証完了後にPW変更ページへ遷移 |
...
| 機能 | 実装詳細 |
|------|----------|
| 履歴書管理 | `ResumeDto`一つでフォーム全体を受信 → `@Transactional`内で既存データ全削除（delete） → flush → 7つのEntityに新規insertする全置換方式で実装。中間失敗時の全ロールバック保証。連絡先公開可否・履歴書公開可否・スカウト受信同意設定含む |
...
| 機能 | 実装詳細 |
|------|----------|
| イベント基盤の通知システム | ビジネスロジック実行時にサーバーサイドで通知データを生成しDBに保存。ページロード時に`/api/notifications/unread-count`を呼び出しヘッダーベルアイコンバッジに未読数を表示。ユーザーがアイコンクリック時に`/api/notifications`で通知リスト照会。各通知にターゲットURLを保存しクリック時に該当ページへ直接遷移 |
...
| **Service Layer** | 12のサービスクラスで構成された核心ビジネスロジック階層。求職者/求人者管理、求人検索、チャット（STOMP/WebSocket）、通知（イベントトリガーベースのREST API）、メール認証等を処理 |
